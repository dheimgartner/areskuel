...
The goal of `areskuel` (pronunciation: R SQL) is to easily set up, manage and interact with `podman` containers that run `Microsoft SQL Server` (and possibly other containerized DBMS in the future).

## Prerequisites

You need to install `podman` and the `ODBC` drivers. The main functionality is implemented in the bash shell script in `inst/scripts/mssqlcontainer.sh`. That being said, it should be clear that `areskuel` is for Linux-users...
...
5. Run SQL commands.
6. Disconnect.
7. If the container is no longer needed, delete it (all data will be lost!).
